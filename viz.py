...
if __name__ == "__main__":
    ...
    calib_label = utils.Label(calib_path, flodir, verbose=1).label["calib"]
    ...
    viz_video = utils.FlowViz(vid_labelpath, flodir, vector_step=step, use_quiver=True, use_color=2, color_type="mag",
                              key="video", maxmotion=max_velo_mag_real, crop_window=cropper, post_factor=None,
                              calib=cal_factor, fps=camera_fps, verbose=1)

    # ***** Multi PLOT *****
    # viz_video.multiplot(ext=ext, start_at=0, num_images=100)

    # ***** VIDEO *****
    viz_video.video(start_at=1800, num_images=90, fps=3, ext="mp4")
